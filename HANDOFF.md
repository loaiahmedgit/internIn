...
1. Analytics (PostHog), error tracking (Sentry), background jobs (Inngest for slow AI generation) — none installed, deliberately deferred to last (see cross-cutting note below).

The full company → AI Challenge → student submission → evidence → comparison → invite → accept → program → tasks/feedback → Verified Experience → student portfolio loop (docs/00–04) now has a working vertical slice end to end, backed by a real, live Supabase database, a real OpenRouter key, and real Resend-backed confirmation emails sent from a verified domain (see section 10) — nothing in the core product loop is running on mocks or placeholders anymore. What's left is the rest of Phase 6 plumbing (item 1) — see `docs/08-page-list-and-data-model.md` for anything not yet covered here (Discover/Smart Matching scoring, real Supabase Storage file upload).
