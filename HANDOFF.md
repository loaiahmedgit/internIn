...
1. **Resend is only sending from its sandbox address (`onboarding@resend.dev`), not a real domain.** Confirmation emails send and land in inboxes fine, but a custom domain hasn't been verified in Resend yet — worth doing before this is a real product anyone signs up to (sandbox senders read as spammy and Resend may restrict recipients over time). Once a domain's verified: update `supabase/config.toml`'s `[auth.email.smtp]` `admin_email`/`sender_name`, then `npx supabase config push --project-ref qdlrrqjevcvjtkbsezaj`.
2. Analytics (PostHog), error tracking (Sentry), background jobs (Inngest for slow AI generation) — none installed, deliberately deferred to last (see cross-cutting note below).

The full company → AI Challenge → student submission → evidence → comparison → invite → accept → program → tasks/feedback → Verified Experience → student portfolio loop (docs/00–04) now has a working vertical slice end to end, backed by a real, live Supabase database, a real OpenRouter key, and real Resend-backed confirmation emails (see section 10) — nothing in the core product loop is running on mocks or placeholders anymore. What's left is a real sending domain (item 1) and the rest of Phase 6 plumbing (item 2) — see `docs/08-page-list-and-data-model.md` for anything not yet covered here (Discover/Smart Matching scoring, real Supabase Storage file upload).
...
- **Auth confirmation emails run through Resend**, not Supabase's own limited built-in mailer. `supabase/config.toml` (committed — no secret in it) has `[auth.email.smtp]` pointing at `smtp.resend.com`, `pass = "env(RESEND_API_KEY)"` — the CLI reads `RESEND_API_KEY` from the shell environment **at push time** and substitutes it; it's never written into the file. `RESEND_API_KEY` lives in `.env.local` only. To change anything here: edit `config.toml`, then `set -a; source .env.local; set +a; npx supabase config push --project-ref qdlrrqjevcvjtkbsezaj` (the CLI needs `RESEND_API_KEY` in its own process env for that `env()` substitution — sourcing `.env.local` first is required, `next dev`/`next build` loading it automatically doesn't help here since this is a separate CLI invocation).
  ...
  - Sender is currently `onboarding@resend.dev` (Resend's shared sandbox address — works, but reads as sandboxy and Resend may restrict recipients on it long-term). See item 1 in section 6 for verifying a real domain.
