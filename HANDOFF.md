...
- **DB schema**: 17 tables, two migrations generated (`0000_zippy_terrax.sql`, `0001_blushing_morgan_stark.sql` — adds `submissions.notes`) but **never pushed to a live database** — no Supabase project has been connected in this session.
...
1. **Nothing has touched a live Postgres database yet.** No `DATABASE_URL`/Supabase project has been provided. Get real credentials into `.env.local` (see `.env.local.example`), then `npx drizzle-kit push` (or apply the generated migrations) before anything DB-backed can be tested for real.
2. Email (Resend), analytics (PostHog), error tracking (Sentry), background jobs (Inngest for slow AI generation) — none installed, deliberately deferred to last (see cross-cutting note below).

The full company → AI Challenge → student submission → evidence → comparison → invite → accept → program → tasks/feedback → Verified Experience → student portfolio loop (docs/00–04) now has a working vertical slice end to end. What's left is infrastructure (item 1) and Phase 6 plumbing (item 2) — see `docs/08-page-list-and-data-model.md` for anything not yet covered here (student profile editing, Discover/Smart Matching scoring, real Supabase Storage file upload).
...
Env vars: copy `.env.local.example` → `.env.local`. The app runs and demos fully with **zero** env vars set (mock AI provider, but auth/dashboard/wizard persistence will fail closed — `proxy.ts` redirects to `/signin` for protected routes even without Supabase configured, and DB actions throw a clear "DATABASE_URL is not set" error rather than crashing silently). To make Phase 2 fully live: real Supabase project (`DATABASE_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`), then `npx drizzle-kit push`. Real AI: `OPENROUTER_API_KEY` (swaps in automatically, no code change).
