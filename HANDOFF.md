...
R4 application foundation is in the working tree: profile + short work responses, transparent company-defined major/education/certification prerequisites, optional CV, canonical application retries, immutable started Challenge version, student/company evidence display, and matching resource authorization. Migrations `0029_application_evidence_and_attempts.sql` and `0030_universal_challenge_architect.sql` are now applied to production and were rerun successfully for idempotency. R4 code remains uncommitted/unreleased. Its rolled-back database verification passed 21 assertions (including idempotency, duplicate constraints, frozen evidence/assignment and student/company RLS); the entire migration and synthetic fixtures were rolled back. No historical application was assigned fabricated evidence. Architect AI/manual authoring, adaptive critical questions, entry foundations/evidence mapping, direct-care limitations and approval gates are implemented in the working tree. R4 local browser QA passed manual authoring/publication, company prerequisite denial before questions, CV-free baseline application, mobile refresh, explicit start, original-version persistence after a published revision, final submission against that original version, company evidence display, and cross-student 404 denial. Release checks are in progress; R5–R7 and post-hire expansion remain outstanding. Continue the owner roadmap automatically.

Temporary R4 QA fixtures currently exist: two synthetic postings, two applications, one submission, one Challenge with immutable versions, and one isolated `Roadmap QA — synthetic only` company and three synthetic auth/app users. The owner explicitly approved temporarily verifying only this QA company (1191921a-c0d3-4903-a172-173ab81aea99); that change is applied and will be removed with cleanup. Exact account IDs and private browser states are in `/tmp/internin-roadmap-qa.json` and sibling state files; never print their credentials. `scripts/cleanup-roadmap-qa.mjs` verifies identities/non-QA dependents before removal. Cleanup and deletion of temporary QA scripts/state files remain mandatory. Baseline real counts were 46 users, 2 companies, 11 opportunities, 45 applications, 0 credentials and 6 programs. Playwright sessions `internin-r4` (production) and `internin-r4-local` (local port 3100) use only these synthetic sessions.
...
**Current phase: R4 — Universal Challenge Architect — RELEASE VERIFICATION.**

Final verification: 510 non-live tests passed across 52 files, the bounded live Architect contract passed, changed-source lint passed, and the production webpack build passed. The paced bulk live result remains quota-limited as documented above.

Implementation and local functional QA are complete; no R4 release is claimed until the exact pushed SHA is READY and checked in production. Migrations 0029 and 0030 are already applied and idempotency-checked; historical evidence remains untouched. The top active checkpoint supersedes the older pre-release notes throughout this file.

Known limitations: free Groq quota interrupted the bulk live suite; semantic AI output remains proposed and requires human review. Deterministic checks in an assessment plan are proposals, not executed validators. Completion windows are guidance, not timed cutoff enforcement. Generated resources and fair variants are R5; evidence validators and fast review are R6. No new paid integration or external repository is required.

Next exact actions: finish final focused checks, commit/push R4, verify the exact deployment, repeat key synthetic flows against production, delete all exact QA fixtures and temporary scripts/private states, record cleanup, then continue automatically with R5 Resource Factory + Fair Variants.
