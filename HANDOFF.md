...
- **Vercel**: production is live at `https://internin-six.vercel.app` (project `internin` under `loaiahmedgits-projects` — Vercel appended `-six` since the bare `internin` name was already taken by someone else on the platform). Deployed via `vercel deploy --prod --token <token>` (CLI, not the dashboard).
  - All 12 app env vars are set on Vercel's **Production** environment (`vercel env ls` to check names — never prints values): `DATABASE_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `OPENROUTER_API_KEY`, `AI_MODEL`, `RESEND_API_KEY`, `NEXT_PUBLIC_POSTHOG_KEY`, `NEXT_PUBLIC_POSTHOG_HOST`, `SENTRY_DSN`, `NEXT_PUBLIC_SENTRY_DSN`, `INNGEST_EVENT_KEY`, `INNGEST_SIGNING_KEY`, `NEXT_PUBLIC_APP_URL` (set to the production URL itself). **`INNGEST_DEV` is deliberately NOT set on Vercel** — that flag only belongs in local `.env.local` (see the Inngest gotcha above); if it were set on the real deployment, production would silently think it's talking to a local dev server and never reach real Inngest Cloud.
  ...
  - `supabase/config.toml`'s `auth.site_url` now points at the production URL (confirmation/reset email links go to real users' actual site, not `localhost`); `additional_redirect_urls` keeps `http://localhost:3000` so local dev auth still works. Pushed the same way as the Resend SMTP change (read the diff first).
  - **Not done, needs a human in a browser**: GitHub auto-deploy isn't connected — both `vercel link` and `vercel git connect` failed with `Failed to connect loaiahmedgit/internIn to project`, because Vercel's GitHub App has never been authorized for this account (that's a one-time OAuth/App-install step in the Vercel dashboard, not scriptable via CLI/token). Until that's done, new commits to `main` won't auto-deploy — re-run `vercel deploy --prod --token <token>` manually after each change, or connect Git via Vercel's dashboard (Project → Settings → Git) once.
  - **Also not done, needs a human in a browser**: Inngest Cloud doesn't know about the production URL yet. Locally, `inngest-cli dev` auto-discovers the app; in production, Inngest Cloud needs to be told about `https://internin-six.vercel.app/api/inngest` once — either via the Inngest dashboard's "Sync new app" (paste that URL) or by installing Inngest's official Vercel integration (which also auto-resyncs on every future deploy). Until synced, background jobs sent via `inngest.send()` on production will queue on Inngest's side but never actually invoke the deployed function.
