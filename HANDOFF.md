...
The owner selected Groq and provided a temporary test key. `.env.local` now explicitly selects Groq with `openai/gpt-oss-120b`; the official `@ai-sdk/groq` adapter is installed. Provider keys stay server-side and are never committed. Matching Vercel production environment settings are active. Groq release `8dfbbaecc06cfe6f3df30c13df69a6d73925ed9b` is production READY (`dpl_HRuLPgh6LrG3TUUMR4V1hKeF8CEh`), and authenticated browser QA returned the correct empty-workspace summary. OpenRouter remains an explicitly selectable alternative; it returned HTTP 402 on this account. A missing key or provider outage never produces mock evidence in production.

Groq compatibility validation: one isolated run passed all 24 live router, role-intelligence and clarification-wording regression cases together. Subsequent full runs exposed rate-limit timeouts and intermittent extraction/wording quality failures; these remain under investigation and are not waived. The extraction prompt now preserves only stated duties/tools and correctly handles a title conflicting with actual work. Nine provider selection/transport tests cover schema validation, hidden reasoning and no mock fallback. Groq itself is live at the exact SHA above. The later paced full run had 523 passing, 9 failing and 1 pending tests after the free 200,000-token daily limit was reached; it is not a clean full-suite result. Do not repeat bulk live tests while quota is constrained. A bounded R4 Architect live contract test subsequently passed after replacing repeated foundation names with validated numbered references.

R4 application foundation is in the working tree: profile + short work responses, transparent company-defined major/education/certification prerequisites, optional CV, canonical application retries, immutable started Challenge version, student/company evidence display, and matching resource authorization. Migrations `0029_application_evidence_and_attempts.sql` and `0030_universal_challenge_architect.sql` are now applied to production and were rerun successfully for idempotency. R4 code is released at the exact SHA below. Its rolled-back database verification passed 21 assertions (including idempotency, duplicate constraints, frozen evidence/assignment and student/company RLS); the entire migration and synthetic fixtures were rolled back. No historical application was assigned fabricated evidence. Architect AI/manual authoring, adaptive critical questions, entry foundations/evidence mapping, direct-care limitations and approval gates are implemented in the working tree. R4 local browser QA passed manual authoring/publication, company prerequisite denial before questions, CV-free baseline application, mobile refresh, explicit start, original-version persistence after a published revision, final submission against that original version, company evidence display, and cross-student 404 denial. Release checks are complete; R5–R7 and post-hire expansion remain outstanding. Continue the owner roadmap automatically.

R4 production release verified READY: `0106e74e155a3170421e5e492ba254eadbc72ff4`, deployment `dpl_EEc44uMPgcuDBgP72MWNaKBSsTKu`. Authenticated production QA passed original-version/final-submission/baseline display and the mobile Quick Apply workspace with no horizontal overflow. The exact synthetic company temporarily verified with the owner's explicit approval, its three auth/app users, two postings, two applications, one submission, Challenge versions and events were then deleted and verified absent. Temporary QA scripts and private account/state files were removed. Real aggregate counts restored: 46 users, 2 companies, 11 opportunities, 45 applications, 0 credentials, 6 programs. No QA fixtures remain.
...
**R4 — Universal Challenge Architect — COMPLETE AND LIVE.**

Release: `0106e74e155a3170421e5e492ba254eadbc72ff4`, deployment `dpl_EEc44uMPgcuDBgP72MWNaKBSsTKu`, READY. Migrations 0029 and 0030 applied and idempotency-checked. Final checks: 510 non-live tests passed across 52 files; bounded live Architect contract passed; changed-source lint and production webpack build passed. Local end-to-end and exact-deployment production QA passed; all synthetic fixtures removed and real baseline counts restored (see top checkpoint).

Known limitations: free Groq quota interrupted the bulk live suite; that run was not fully green. AI output remains proposed and requires human review. Deterministic checks in an assessment plan are proposals, not executed validators. Completion windows are guidance, not timed cutoff enforcement. No overall suitability score or new credential attestation was introduced.

**NEXT TASK: R5 — Resource Factory + Fair Variants. Continue automatically.**

Owner brief sections 20–28 and 39 have been read. Existing `resource-generation.ts` supports real CSV/TXT/MD/JSON/PDF/XLSX/DOCX bytes but its generic spreadsheet cells and no-spec fallbacks are inadequate for known ground truth. Existing upload finalization trusts caller path/size and can replace resources on a frozen version; fix those boundaries in R5. Introduce ResourceSpec/origin/purpose/foundation metadata, deterministic template adapters and private answer keys, managed shared-fact validation, real company preview/sanitized upload flow, and small approved equivalent families with immutable start assignment. Keep the R4 assigned-version authorization and legacy data intact. No paid integration or new external repository is currently needed.
