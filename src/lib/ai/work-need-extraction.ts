...
export function normalizeWorkNeedProfile(profile: WorkNeedProfile, originalRequest: string): WorkNeedProfile {
  ...
  const activityClarity =
    profile.activityClarity === "ambiguous" && hasTwoGroundedActivities(profile, originalRequest)
      ? "clear"
...
- activities are the concrete work needed to address those problems. Activities may make safe, direct implications explicit (for example, migrating inconsistent records implies data preparation, cleansing, mapping, and validation), but must not invent company facts.
