    ...
    case "to_do":
      return (
        <div className="mt-3 border-t border-navy/8 pt-3">
          <p className="text-xs font-medium text-navy/60">Applied — challenge not started</p>
    ...
    case "submitted":
      return (
        <div className="mt-3 border-t border-navy/8 pt-3">
          ...
          <p className="text-xs text-navy/50">Awaiting review — not a hiring decision yet</p>
          <Link href={`/student/applications/${state.applicationId}`} className={buttonClass}>
            View application
          </Link>
        </div>
...
export function OpportunityCard({
  ...
  return (
    <div className="rounded-xl border border-navy/10 bg-white p-5 shadow-[0_1px_2px_rgba(33,50,72,0.04)] transition-shadow hover:shadow-[0_8px_24px_rgba(33,50,72,0.08)]">
      ...
      {opportunity.description && <p className="mt-2 text-sm text-navy/55">{teaser(opportunity.description)}</p>}
      ...
      {challengeRequired && (
        <div className="mt-3 flex items-center gap-1.5 text-xs text-navy/50">
          <Clock className="size-3.5" aria-hidden="true" />
          <span>
            Challenge required{typeof estimatedMinutes === "number" ? ` · ~${estimatedMinutes} min` : ""}
          </span>
        </div>
      )}
