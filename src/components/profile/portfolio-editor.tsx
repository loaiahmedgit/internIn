...
import {
  ...
  getPortfolioAttachmentUploadUrlAction,
  getPortfolioThumbnailUploadUrlAction,
  upsertPortfolioItemAction,
...
// Generic across professions — a portfolio item is not just a "project".
const TYPE_OPTIONS = ["Project", "Case study", "Design", "Research", "Writing", "Presentation", "Campaign", "Video", "Repository", "Document", "Other"];

// A repository link only makes sense for code-shaped work.
const REPOSITORY_TYPES = new Set(["Project", "Repository"]);
...
function emptyDraft(): Draft {
  return { title: "", itemType: TYPE_OPTIONS[0], description: "", thumbnailUrl: "", externalUrl: "", repositoryUrl: "", attachmentUrl: "", dateLabel: "", skillsText: "" };
}
...
export function PortfolioEditor({ items }: { items: PortfolioItem[] }) {
  ...
  const [uploading, setUploading] = useState<"thumbnail" | "attachment" | null>(null);
  ...
  async function handleAttachment(file: File) {
    ...
    setError(null);
    try {
      const { path, token, publicUrl } = await getPortfolioAttachmentUploadUrlAction(file.name);
      const supabase = createClient();
      const { error: uploadError } = await supabase.storage.from("student-portfolio").uploadToSignedUrl(path, token, file);
      if (uploadError) throw new Error(`Couldn't upload "${file.name}": ${uploadError.message}`);
      setDraft((d) => ({ ...d, attachmentUrl: publicUrl }));
    } catch (err) {
    ...
    startTransition(async () => {
      try {
        await upsertPortfolioItemAction({
          ...
          repositoryUrl: REPOSITORY_TYPES.has(draft.itemType ?? "") ? draft.repositoryUrl || undefined : undefined,
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <section id="portfolio" className="scroll-mt-24 rounded-2xl border border-black/[0.04] bg-white p-5 shadow-[0_1px_2px_rgba(16,24,40,0.04),0_8px_24px_-4px_rgba(16,24,40,0.10)]">
        ...
        {items.length > 0 ? (
          ...
          <p className="mt-2 text-sm text-navy/55">
            Show what you&apos;ve built, designed, written, researched, or created.{" "}
            <SheetTrigger onClick={openAdd} className="font-medium text-teal-ink hover:underline">Add work →</SheetTrigger>
          </p>
        )}
      </section>
      ...
      <SheetContent className="flex flex-col gap-0 overflow-y-auto p-0 data-[side=right]:w-full data-[side=right]:sm:max-w-md">
        ...
        <div className="flex-1 space-y-4 px-5 py-5">
          ...
          <div>
            <label htmlFor="pf-thumb" className="text-sm font-medium text-navy">Thumbnail / cover image (optional)</label>
            {draft.thumbnailUrl && (
            ...
            <input
              ...
              onChange={(e) => { const file = e.target.files?.[0]; if (file) handleThumbnail(file); e.target.value = ""; }}
              className="mt-1.5 text-sm text-navy/70"
            />
            {uploading === "thumbnail" && <p className="mt-1 text-xs text-navy/50">Uploading…</p>}
          </div>
          <div>
            <label htmlFor="pf-external" className="text-sm font-medium text-navy">Primary link (optional)</label>
            ...
            <p className="mt-1 text-xs text-navy/50">Wherever this work lives — a live site, a Figma file, a published article, a case study page.</p>
          </div>
          {REPOSITORY_TYPES.has(draft.itemType ?? "") && (
            <div>
              <label htmlFor="pf-repo" className="text-sm font-medium text-navy">Repository URL (optional)</label>
          ...
          <div>
            <label htmlFor="pf-attachment" className="text-sm font-medium text-navy">Attachment (optional)</label>
            <p className="mt-1 text-xs text-navy/50">A PDF — a writing sample, a research paper, a case study document.</p>
            {draft.attachmentUrl && (
              <a href={draft.attachmentUrl} target="_blank" rel="noreferrer" className="mt-1.5 flex items-center gap-1.5 text-xs font-medium text-teal-ink hover:underline">
            ...
            <input
              ...
              onChange={(e) => { const file = e.target.files?.[0]; if (file) handleAttachment(file); e.target.value = ""; }}
              className="mt-1.5 text-sm text-navy/70"
            />
            {uploading === "attachment" && <p className="mt-1 text-xs text-navy/50">Uploading…</p>}
          </div>
          <div>
          ...
          <div className="flex items-center gap-2 pt-1">
            <Button type="button" onClick={save} disabled={isPending || uploading !== null} className="h-9 bg-teal text-white hover:bg-teal-ink">{isPending ? "Saving…" : "Save"}</Button>
